...
export default class FeaturedItem2 extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:3001/api/featureditem1')
            .then(response => {
                this.setState({
                    feat_items: response.data[1], feat_name: response.data[1].item_name,
                    feat_desc: response.data[1].item_description, feat_specs: response.data[1].specs,
                    feat_price: response.data[1].price, feat_link: response.data[1].link,
                    feat_url: response.data[1].item_url, edited_item_id: response.data[1].id})
                }).then( () =>
